...
class Template(commands.Cog):
    ...
    @cog_ext.cog_slash(
        ...
        create_option(
            ...
            option_type=3,
            required=False,
            choices= [create_choice(name=s['name'],value=s['name']) for s in STYLES]
        ),
        create_option(
            name="glow",
    ...
    async def template(self,ctx,image_url,style_name,glow,title,ox,oy,nocrop):
        ...
        template_array = await self.client.loop.run_in_executor(None,templatize,style,img_array, glow_opacity)
