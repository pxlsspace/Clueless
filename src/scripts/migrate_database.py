...
async def migrate_pxls_user_stats():
    ...
    with alive_bar(0,title="Fetching old db data",bar='classic',spinner='classic') as bar: 
        old_cur.execute(sql)
    ...
    with alive_bar(total_lines,title="Groupping by date",bar='classic',spinner='classic') as bar: 
    ...
    async with db_conn.conn.cursor() as cur:
        with alive_bar(total_lines,title="Formating users",bar='classic',spinner='classic') as bar: 
        ...
        sql = """ INSERT INTO pxls_user_stat (record_id, pxls_name_id, alltime_count, canvas_count)
                  VALUES (?,?,?,?)"""
        with alive_bar(0,title=f"Inserting {len(values_list)} lines",bar='classic',spinner='classic') as bar:
...
async def migrate_general_stats():
    with alive_bar(0,title="Inserting general stats",bar='classic',spinner='classic') as bar: 
        ...
        async with db_conn.conn.cursor() as curs:
            ...
            await curs.executemany("INSERT INTO pxls_general_stat(stat_name,value,canvas_code,datetime) VALUES (?,?,?,?)",values_list)
        await db_conn.conn.commit()
        await db_conn.close_connection()
        bar()
