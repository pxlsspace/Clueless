...
def get_progress(template_array: np.ndarray, params, get_progress_image = True):
    ...
    cropped_board[y0-y:(y0-y)+_cropped_board.shape[0],x0-x:(x0-x)+_cropped_board.shape[1]] = _cropped_board
    cropped_placemap = np.full_like(palettized_array,255)
    cropped_placemap[y0-y:(y0-y)+_cropped_placemap.shape[0],x0-x:(x0-x)+_cropped_placemap.shape[1]] = _cropped_placemap
    ...
    correct_pixels = np.sum(placed_mask==True) # count the "True" pixels on the mask

    if get_progress_image:
        progress_array = np.zeros((height,width,4),dtype=np.uint8)
        opacity = 0.65
        progress_array[placed_mask==True] = [0,255,0,255*opacity] # correct pixels = green
        progress_array[placed_mask==False] = [255,0,0,255*opacity] # incorrect pixels = red
